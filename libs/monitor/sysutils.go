...
type DiskAdvacedInfo struct {
	...
	Usage        *disk.UsageStat                `json:"lib_usage"`
}
...
type CpuInfo struct {
	CpuInfos     []CpuInfo     `json:"cpu_infos"`
	PhysicalCnt  int           `json:"physical_cnt"`
	LogicalCnt   int           `json:"logical_cnt"`
	TotalPercent []float64     `json:"total_percent"`
	PerPercents  []float64     `json:"per_percents"`
...
func SysCpuInfo() (CpuInfo, error) {
	var (
		err     error
		cpuInfo CpuInfo
	)
	cpuInfo.PhysicalCnt, err = cpu.Counts(false)
	if err != nil {
		return cpuInfo, err
	}
	cpuInfo.LogicalCnt, err = cpu.Counts(true)
	if err != nil {
		return cpuInfo, err
	}

	cpuInfo.TotalPercent, err = cpu.Percent(1*time.Second, false)
	if err != nil {
		return cpuInfo, err
	}
	cpuInfo.PerPercents, err = cpu.Percent(1*time.Second, true)
	cpuInfo.LoadAvg, err = load.Avg()
	if err != nil {
		return cpuInfo, err
	}
	return cpuInfo, nil

...
type IronCoreOblong struct {
	ID                      int       `json:"id" gorm:"column:id;primary_key"`                                       // 铁芯（长圆形）编号
	...
	StackWidth0             float32   `json:"stack_width_0" gorm:"column:stack_width_0"`                             // 片宽（0级）
	...
	StackThick0             float32   `json:"stack_thick_0" gorm:"column:stack_thick_0"`                             // 迭厚（0级）
	...
	MainLevelRealStackThick float64   `json:"main_level_real_stack_thick" gorm:"column:main_level_real_stack_thick"` // 主级实迭厚
	LicoCreator             int       `json:"lico_creator" gorm:"column:lico_creator"`                               // 创建者
	CreatorUser             *User     `json:"creator_user,omitempty" gorm:"ForeignKey:LicoCreator;AssociationForeignKey:ID"`
...
func (IronCoreOblong) TableName() string {
	return "lib_iron_core_oblong"
}

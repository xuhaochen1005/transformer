...
import { TagViewAction } from '@/store/tagsview/action'
import { TagViewMutation } from '@/store/tagsview/mutation'
import { TagView } from '@/store/state'
  ...
  setup() {
    ...
    const state = reactive({
      ...
      refreshSelectedTag: (view: TagView) => {
        store.commit(TagViewMutation.DEL_CACHED_VIEW, view)
        const { fullPath } = view
        nextTick(() => {
      ...
      closeSelectedTag: (view: TagView) => {
        store.dispatch(TagViewAction.DEL_VIEW, view)
      ...
      closeOthersTags: () => {
        ...
        store.dispatch(TagViewAction.DEL_OTHER_VIEW, state.selectedTag as TagView)
      },
      closeAllTags: (view: TagView) => {
        store.dispatch(TagViewAction.DEL_ALL_VIEWS, undefined)
    ...
    const routes = computed(() => permissionRoutes)
    ...
    const initTags = () => {
      ...
      for (const tag of state.affixTags) {
        // Must have tag name
        if (tag.name) {
          store.commit(TagViewMutation.ADD_VISITED_VIEW, tag as TagView)
        }
      }
    }
    ...
    const addTags = () => {
      if (currentRoute.name) {
        store.dispatch(TagViewAction.ADD_VIEW, currentRoute)
      }
      return false
    }
        ...
        for (const tag of tags) {
          if ((tag.to as TagView).path === currentRoute.path) {
            ...
            if ((tag.to as TagView).fullPath !== currentRoute.fullPath) {
              store.commit(TagViewMutation.UPDATE_VISITED_VIEW, currentRoute)
            }
          }
        }
      })
